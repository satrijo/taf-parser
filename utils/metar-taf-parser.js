...
class CeilingHeightCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _CeilingHeightCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _CeilingHeightCommand_regex, "f")
    );
...
class CeilingSecondLocationCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _CeilingSecondLocationCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _CeilingSecondLocationCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class HailSizeCommand extends Command {
  ...
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HailSizeCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    ...
    return [
      code
        .replace(__classPrivateFieldGet(this, _HailSizeCommand_regex, "f"), "")
        .trim(),
      remark,
    ];
  }
...
class HourlyMaximumMinimumTemperatureCommand extends Command {
  constructor() {
    super(...arguments);
    _HourlyMaximumMinimumTemperatureCommand_regex.set(
      this,
      /^4([01])(\d{3})([01])(\d{3})\b/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _HourlyMaximumMinimumTemperatureCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
...
class HourlyMaximumTemperatureCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _HourlyMaximumTemperatureCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HourlyMaximumTemperatureCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
...
class HourlyMinimumTemperatureCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _HourlyMinimumTemperatureCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HourlyMinimumTemperatureCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
...
class HourlyPrecipitationAmountCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _HourlyPrecipitationAmountCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HourlyPrecipitationAmountCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class HourlyPressureCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _HourlyPressureCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HourlyPressureCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class HourlyTemperatureDewPointCommand extends Command {
  constructor() {
    super(...arguments);
    _HourlyTemperatureDewPointCommand_regex.set(
      this,
      /^T([01])(\d{3})(([01])(\d{3}))?/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _HourlyTemperatureDewPointCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _HourlyTemperatureDewPointCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class IceAccretionCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _IceAccretionCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _IceAccretionCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
...
class ObscurationCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _ObscurationCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _ObscurationCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class PrecipitationAmount24HourCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrecipitationAmount24HourCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrecipitationAmount24HourCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class PrecipitationAmount36HourCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrecipitationAmount36HourCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrecipitationAmount36HourCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class PrecipitationBegEndCommand extends Command {
  constructor() {
    super(...arguments);
    _PrecipitationBegEndCommand_regex.set(
      this,
      /^(([A-Z]{2})?([A-Z]{2})B(\d{2})?(\d{2})E(\d{2})?(\d{2}))/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrecipitationBegEndCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrecipitationBegEndCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class PrevailingVisibilityCommand extends Command {
  constructor() {
    super(...arguments);
    _PrevailingVisibilityCommand_regex.set(
      this,
      /^VIS ((\d)*( )?(\d?\/?\d))V((\d)*( )?(\d?\/?\d))/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrevailingVisibilityCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrevailingVisibilityCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SeaLevelPressureCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _SeaLevelPressureCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SeaLevelPressureCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SecondLocationVisibilityCommand extends Command {
  constructor() {
    super(...arguments);
    _SecondLocationVisibilityCommand_regex.set(
      this,
      /^VIS ((\d)*( )?(\d?\/?\d)) (\w+)/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _SecondLocationVisibilityCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SecondLocationVisibilityCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SectorVisibilityCommand extends Command {
  constructor() {
    super(...arguments);
    _SectorVisibilityCommand_regex.set(
      this,
      /^VIS ([A-Z]{1,2}) ((\d)*( )?(\d?\/?\d))/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _SectorVisibilityCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SectorVisibilityCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SmallHailSizeCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _SmallHailSizeCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SmallHailSizeCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
...
class SnowDepthCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _SnowDepthCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SnowDepthCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    ...
    return [
      code
        .replace(__classPrivateFieldGet(this, _SnowDepthCommand_regex, "f"), "")
        .trim(),
      remark,
    ];
  }
...
class SnowIncreaseCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _SnowIncreaseCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SnowIncreaseCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SnowPelletsCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _SnowPelletsCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SnowPelletsCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
      _("Remark.Snow.Pellets", this.locale),
      _(`Remark.${matches[1]}`, this.locale)
    );
    remark.push({
...
class SunshineDurationCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _SunshineDurationCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SunshineDurationCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class SurfaceVisibilityCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _SurfaceVisibilityCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _SurfaceVisibilityCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const distance = matches[1];
    const description = format(
...
class ThunderStormLocationCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _ThunderStormLocationCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _ThunderStormLocationCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class ThunderStormLocationMovingCommand extends Command {
  constructor() {
    super(...arguments);
    _ThunderStormLocationMovingCommand_regex.set(
      this,
      /^TS ([A-Z]{2}) MOV ([A-Z]{2})/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _ThunderStormLocationMovingCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
...
class TornadicActivityBegCommand extends Command {
  constructor() {
    super(...arguments);
    _TornadicActivityBegCommand_regex.set(
      this,
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _TornadicActivityBegCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _TornadicActivityBegCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const direction = as(matches[7], Direction);
    const description = format(
      _("Remark.Tornadic.Activity.Beginning", this.locale),
      _(`Remark.${matches[1].replace(" ", "")}`, this.locale),
      matches[3] || "",
      matches[4],
      matches[6],
      _(`Converter.${direction}`, this.locale)
...
class TornadicActivityBegEndCommand extends Command {
  constructor() {
    super(...arguments);
    _TornadicActivityBegEndCommand_regex.set(
      this,
  ...
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _TornadicActivityBegEndCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class TornadicActivityEndCommand extends Command {
  constructor() {
    super(...arguments);
    _TornadicActivityEndCommand_regex.set(
      this,
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _TornadicActivityEndCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _TornadicActivityEndCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const direction = as(matches[7], Direction);
    const description = format(
...
class TowerVisibilityCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _TowerVisibilityCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _TowerVisibilityCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const distance = matches[1];
    const description = format(
...
class VariableSkyCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _VariableSkyCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _VariableSkyCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class VariableSkyHeightCommand extends Command {
  constructor() {
    super(...arguments);
    _VariableSkyHeightCommand_regex.set(
      this,
      /^([A-Z]{3})(\d{3}) V ([A-Z]{3})/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _VariableSkyHeightCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _VariableSkyHeightCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const firstQuantity = as(matches[1], CloudQuantity);
...
class VirgaDirectionCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _VirgaDirectionCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _VirgaDirectionCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const direction = as(matches[1], Direction);
    const description = format(
...
class WaterEquivalentSnowCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _WaterEquivalentSnowCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _WaterEquivalentSnowCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class WindPeakCommand extends Command {
  constructor() {
    super(...arguments);
    _WindPeakCommand_regex.set(
      this,
      /^PK WND (\d{3})(\d{2,3})\/(\d{2})?(\d{2})/
    );
  }
  canParse(code) {
  ...
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _WindPeakCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    ...
    return [
      code
        .replace(__classPrivateFieldGet(this, _WindPeakCommand_regex, "f"), "")
        .trim(),
      remark,
    ];
  }
...
class WindShiftCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _WindShiftCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _WindShiftCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
    ...
    return [
      code
        .replace(__classPrivateFieldGet(this, _WindShiftCommand_regex, "f"), "")
        .trim(),
      remark,
    ];
  }
...
class WindShiftFropaCommand extends Command {
  ...
  canParse(code) {
    return __classPrivateFieldGet(this, _WindShiftFropaCommand_regex, "f").test(
      code
    );
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _WindShiftFropaCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const description = format(
...
class PrecipitationBegCommand extends Command {
  constructor() {
    super(...arguments);
    _PrecipitationBegCommand_regex.set(
      this,
      /^(([A-Z]{2})?([A-Z]{2})B(\d{2})?(\d{2}))/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrecipitationBegCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrecipitationBegCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const descriptive = matches[2] ? as(matches[2], Descriptive) : undefined;
    const phenomenon = as(matches[3], Phenomenon);
...
class PrecipitationEndCommand extends Command {
  constructor() {
    super(...arguments);
    _PrecipitationEndCommand_regex.set(
      this,
      /^(([A-Z]{2})?([A-Z]{2})E(\d{2})?(\d{2}))/
    );
  }
  canParse(code) {
    return __classPrivateFieldGet(
      this,
      _PrecipitationEndCommand_regex,
      "f"
    ).test(code);
  }
  execute(code, remark) {
    const matches = code.match(
      __classPrivateFieldGet(this, _PrecipitationEndCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    const descriptive = matches[2] ? as(matches[2], Descriptive) : undefined;
    const phenomenon = as(matches[3], Phenomenon);
...
(function (RemarkType) {
  ...
  RemarkType["HourlyMaximumMinimumTemperature"] =
    "HourlyMaximumMinimumTemperature";
...
var _CloudCommand_cloudRegex,
  _MainVisibilityCommand_regex,
  _WindCommand_regex,
  _WindVariationCommand_regex,
  _WindShearCommand_regex,
  _VerticalVisibilityCommand_regex,
  _MinimalVisibilityCommand_regex,
  _MainVisibilityNauticalMilesCommand_regex,
  _CommandSupplier_commands$2;
/**
...
class CloudCommand {
  constructor() {
    _CloudCommand_cloudRegex.set(
      this,
      /^([A-Z]{3})(?:\/{3}|(\d{3}))?(?:\/{3}|(?:([A-Z]{2,3})(?:\/([A-Z]{2,3}))?))?$/
    );
  }
  parse(cloudString) {
    const m = cloudString.match(
      __classPrivateFieldGet(this, _CloudCommand_cloudRegex, "f")
    );
    if (!m) return;
  ...
  canParse(cloudString) {
    ...
    return __classPrivateFieldGet(this, _CloudCommand_cloudRegex, "f").test(
      cloudString
    );
  }
}
...
class MainVisibilityCommand {
  ...
  canParse(visibilityString) {
    return __classPrivateFieldGet(this, _MainVisibilityCommand_regex, "f").test(
      visibilityString
    );
  }
  execute(container, visibilityString) {
    const matches = visibilityString.match(
      __classPrivateFieldGet(this, _MainVisibilityCommand_regex, "f")
    );
...
class WindCommand {
  constructor() {
    _WindCommand_regex.set(
      this,
      /^(VRB|00|[0-3]\d{2})(\d{2})G?(\d{2})?(KT|MPS|KM\/H)?/
    );
  }
  canParse(windString) {
    return __classPrivateFieldGet(this, _WindCommand_regex, "f").test(
      windString
    );
  }
  parseWind(windString) {
    const matches = windString.match(
      __classPrivateFieldGet(this, _WindCommand_regex, "f")
    );
...
class WindVariationCommand {
  ...
  canParse(windString) {
    return __classPrivateFieldGet(this, _WindVariationCommand_regex, "f").test(
      windString
    );
  }
  parseWindVariation(wind, windString) {
    const matches = windString.match(
      __classPrivateFieldGet(this, _WindVariationCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Wind should be defined");
...
class WindShearCommand {
  constructor() {
    _WindShearCommand_regex.set(
      this,
      /^WS(\d{3})\/(\w{3})(\d{2})G?(\d{2})?(KT|MPS|KM\/H)/
    );
  }
  canParse(windString) {
    return __classPrivateFieldGet(this, _WindShearCommand_regex, "f").test(
      windString
    );
  }
  parseWindShear(windString) {
    const matches = windString.match(
      __classPrivateFieldGet(this, _WindShearCommand_regex, "f")
    );
    if (!matches)
...
class VerticalVisibilityCommand {
  ...
  execute(container, visibilityString) {
    const matches = visibilityString.match(
      __classPrivateFieldGet(this, _VerticalVisibilityCommand_regex, "f")
    );
    if (!matches)
...
class MinimalVisibilityCommand {
  ...
  execute(container, visibilityString) {
    const matches = visibilityString.match(
      __classPrivateFieldGet(this, _MinimalVisibilityCommand_regex, "f")
    );
    if (!matches)
      throw new UnexpectedParseError("Vertical visibility should be defined");
...
class MainVisibilityNauticalMilesCommand {
  constructor() {
    _MainVisibilityNauticalMilesCommand_regex.set(
      this,
      /^(P|M)?(\d)*(\s)?((\d\/\d)?SM)$/
    );
  }
...
class CommandSupplier$2 {
  ...
  get(input) {
    for (const command of __classPrivateFieldGet(
      this,
      _CommandSupplier_commands$2,
      "f"
    )) {
...
class AltimeterCommand {
  ...
  canParse(input) {
    return __classPrivateFieldGet(this, _AltimeterCommand_regex, "f").test(
      input
    );
  }
  execute(metar, input) {
    const matches = input.match(
      __classPrivateFieldGet(this, _AltimeterCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
    metar.altimeter = {
      value: +matches[1],
...
class AltimeterMercuryCommand {
  ...
  canParse(input) {
    return __classPrivateFieldGet(
      this,
      _AltimeterMercuryCommand_regex,
      "f"
    ).test(input);
  }
  execute(metar, input) {
    const matches = input.match(
      __classPrivateFieldGet(this, _AltimeterMercuryCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
var _RunwayCommand_genericRegex,
  _RunwayCommand_runwayMaxRangeRegex,
  _RunwayCommand_runwayRegex,
  _RunwayCommand_runwayDepositRegex;
class RunwayCommand {
  constructor() {
    _RunwayCommand_genericRegex.set(this, /^(R\d{2}\w?\/)/);
    _RunwayCommand_runwayMaxRangeRegex.set(
      this,
      /^R(\d{2}\w?)\/(\d{4})V(\d{3,4})([UDN])?(FT)?/
    );
    _RunwayCommand_runwayRegex.set(
      this,
      /^R(\d{2}\w?)\/([MP])?(\d{4})([UDN])?(FT)?$/
    );
    _RunwayCommand_runwayDepositRegex.set(
      this,
      /^R(\d{2}\w?)\/([/\d])([/\d])(\/\/|\d{2})(\/\/|\d{2})$/
    );
  }
  canParse(input) {
    return __classPrivateFieldGet(this, _RunwayCommand_genericRegex, "f").test(
      input
    );
  }
  execute(metar, input) {
    if (
      __classPrivateFieldGet(this, _RunwayCommand_runwayDepositRegex, "f").test(
        input
      )
    ) {
      const matches = input.match(
        __classPrivateFieldGet(this, _RunwayCommand_runwayDepositRegex, "f")
      );
      ...
      __classPrivateFieldGet(this, _RunwayCommand_runwayRegex, "f").test(input)
    ) {
      const matches = input.match(
        __classPrivateFieldGet(this, _RunwayCommand_runwayRegex, "f")
      );
      if (!matches) throw new UnexpectedParseError("Should be able to parse");
      ...
      const matches = input.match(
        __classPrivateFieldGet(this, _RunwayCommand_runwayMaxRangeRegex, "f")
      );
      if (!matches) throw new UnexpectedParseError("Should be able to parse");
      const trend = matches[4] ? as(matches[4], RunwayInfoTrend) : undefined;
      const unit = matches[5]
...
(_RunwayCommand_genericRegex = new WeakMap()),
  (_RunwayCommand_runwayMaxRangeRegex = new WeakMap()),
  (_RunwayCommand_runwayRegex = new WeakMap()),
  (_RunwayCommand_runwayDepositRegex = new WeakMap());
...
class TemperatureCommand {
  ...
  canParse(input) {
    return __classPrivateFieldGet(this, _TemperatureCommand_regex, "f").test(
      input
    );
  }
  execute(metar, input) {
    const matches = input.match(
      __classPrivateFieldGet(this, _TemperatureCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class IcingCommand {
  ...
  execute(container, input) {
    const matches = input.match(
      __classPrivateFieldGet(this, _IcingCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
class TurbulenceCommand {
  ...
  canParse(input) {
    return __classPrivateFieldGet(this, _TurbulenceCommand_regex, "f").test(
      input
    );
  }
  execute(container, input) {
    const matches = input.match(
      __classPrivateFieldGet(this, _TurbulenceCommand_regex, "f")
    );
    if (!matches) throw new UnexpectedParseError("Match not found");
...
var _AbstractParser_INTENSITY_REGEX,
  _AbstractParser_CAVOK,
  _AbstractParser_commonSupplier,
  _MetarParser_commandSupplier,
  _TAFParser_commandSupplier,
  _TAFParser_validityPattern,
  _TAFParser_partialPattern,
  _RemarkParser_supplier;
...
function findFlags(flag) {
  if (flag in FlagMap) return { [FlagMap[flag]]: true };
}
/**
...
class AbstractParser {
  ...
  parseWeatherCondition(input) {
    ...
    if (
      input.match(
        __classPrivateFieldGet(this, _AbstractParser_INTENSITY_REGEX, "f")
      )
    ) {
  ...
  generalParse(abstractWeatherContainer, input) {
    ...
    const command = __classPrivateFieldGet(
      this,
      _AbstractParser_commonSupplier,
      "f"
    ).get(input);
...
(_AbstractParser_INTENSITY_REGEX = new WeakMap()),
  (_AbstractParser_CAVOK = new WeakMap()),
  (_AbstractParser_commonSupplier = new WeakMap());
class MetarParser extends AbstractParser {
  ...
  parseType(token) {
    for (const type in MetarType) {
      if (token === MetarType[type]) return type;
    }
  }
}
...
class TAFParser extends AbstractParser {
  ...
  throwIfPartial(input) {
    ...
    const matches = input.match(
      __classPrivateFieldGet(this, _TAFParser_partialPattern, "f")
    );
    if (matches) {
  ...
  parse(input) {
    ...
    for (let i = index + 1; i < lines[0].length; i++) {
      ...
      const tafCommand = __classPrivateFieldGet(
        this,
        _TAFParser_commandSupplier,
        "f"
      ).get(token);
  ...
  findLineValidity(index, line) {
    ...
    for (let i = index; i < line.length; i++) {
      if (
        __classPrivateFieldGet(this, _TAFParser_validityPattern, "f").test(
          line[i]
        )
      )
...
(_TAFParser_commandSupplier = new WeakMap()),
  (_TAFParser_validityPattern = new WeakMap()),
  (_TAFParser_partialPattern = new WeakMap());
class RemarkParser {
  constructor(locale) {
    this.locale = locale;
  ...
  parse(code) {
    ...
    while (rmkStr) {
      try {
        [rmkStr, rmkList] = __classPrivateFieldGet(
          this,
          _RemarkParser_supplier,
          "f"
        )
          .get(rmkStr)
          .execute(rmkStr, rmkList);
      } catch (e) {
        if (e instanceof CommandExecutionError) {
          [rmkStr, rmkList] = __classPrivateFieldGet(
            ...
            _RemarkParser_supplier,
            "f"
          ).defaultCommand.execute(rmkStr, rmkList);
        } else {
          throw e;
        }
      }
...
function getCompositeForecastForDate(date, forecastContainer) {
  ...
  for (const forecast of forecastContainer.forecast) {
    if (
      ...
      forecast.start.getTime() <= date.getTime() + 3600000
    ) {
